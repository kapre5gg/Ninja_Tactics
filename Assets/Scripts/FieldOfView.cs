...
public class FieldOfView : MonoBehaviour
{
    ...
    private int edgeResolveIterations; // 엣지 해상도
    private float edgeDstThreshold; // 엣지 거리 임계값
    ...
    private Quaternion initialRotation; // 현재 적이 바라보는 방향 저장
    private bool isReturningToInitialAngle = false; // 원래 방향으로 돌아가는 중인지 여부
    private float returnSpeed = 2f; // 초기 각도로 돌아가는 속도
            ...
            if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2) //타겟이 시야각에 들어오면
            {
                ...
                if (!Physics.Raycast(transform.position, dirToTarget, dstToTarget, obstacleMask1)) //시야각에 obstacleMask1 레이어를 가진 오브젝트가 존재 x
                {
                    ...
                    else //시야각에 obstacleMask2 레이어를 가진 오브젝트가 존재 o (obstacleMask1 x, obstacleMask2 o)
                    {
                        ...
                        bool isSitting = ninjaController != null && ninjaController.isSitting; // isCrouch = 플레이어가 앉아있는 상태
                        if (!isSitting) //플레이어가 앉아있는 상태가 아닐 경우 감지됨.
                        {
    ...
    private void DrawFieldOfView()
    {
        ...
        float extendedViewRadius = viewRadius * 1.4f;
